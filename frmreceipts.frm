...
Begin VB.Form frmreceipts 
   ...
   Begin VB.OptionButton Optpaybill 
      Caption         =   "Paybill"
      Height          =   255
      Left            =   2640
      TabIndex        =   88
   ...
   Begin VB.ComboBox ports 
      Height          =   315
      ItemData        =   "frmreceipts.frx":1609
      Left            =   960
      List            =   "frmreceipts.frx":1619
      TabIndex        =   43
   ...
   Begin MSComctlLib.ListView Lvwitems 
      ...
      Appearance      =   1
      MousePointer    =   4
      MouseIcon       =   "frmreceipts.frx":1939
      NumItems        =   6
...
Set rst = oSaccoMaster.GetRecordset("select p_code,Pprice,sprice from ag_products where p_name ='" & cboproductname & "'")
If Not rst.EOF Then
...
rate = 0.75 * lblNPay
If rate < total Then
MsgBox "The total amount should not be more than 75% of netpay", vbInformation
'Exit Sub
End If
...
sql = ""
sql = sql & "SET dateformat DMY INSERT INTO ag_Receipts(R_No, P_code, T_Date, Amount,S_no, Qua, S_Bal, user_id, Cash, SNo,Transby, Idno, mobile) VALUES ("
...
Dim ttt
     ...
     Dim escAlignRight As String
       ...
       Dim fso, chkPrinter, txtFile
        ...
        Do While Not j > (Lvwitems.ListItems.Count)
            Lvwitems.ListItems.Item(j).selected = True
            strReceipts = strReceipts & Lvwitems.SelectedItem.SubItems(1) & vbNewLine & Lvwitems.SelectedItem.SubItems(2) & vbTab
            strReceipts = strReceipts & Format(Lvwitems.SelectedItem.SubItems(3), "#,##0.00") & vbTab & Format(Lvwitems.SelectedItem.SubItems(4), "#,##0.00") & vbNewLine
            j = j + 1
        Loop
...
'nAME QNTY PRICE AMOUNT
    txtFile.WriteLine "QNTY" & vbTab & vbTab & "PRICE" & vbTab & "AMOUNT"
    txtFile.WriteLine "......................................."
...
sql = ""
sql = sql & "SET dateformat DMY INSERT INTO ag_Receipts(R_No, P_code, T_Date, Amount, Qua, S_Bal, user_id, Cash, SNo,Transby, Idno,mobile) VALUES ("
sql = sql & txtrno & ",'" & Lvwitems.SelectedItem & "','" & txtransdate & "'," & Lvwitems.SelectedItem.SubItems(4)
...
txttranscode = "0"
Else
sql = "d_sp_TransDeduct '" & txttranscode & "','" & txtransdate & "','Agrovet'," & Lvwitems.SelectedItem.SubItems(4) & ",'" & Startdate & "','" & Enddate & "','" & User & "','" & Lvwitems.SelectedItem.SubItems(1) & "'"
oSaccoMaster.ExecuteThis (sql)
End If
Next j
...
sql = ""
sql = sql & "SET dateformat DMY INSERT INTO ag_Receipts(R_No, P_code, T_Date, Amount, Qua, S_Bal, user_id, Cash, SNo,Transby) VALUES ("
sql = sql & txtrno & ",'" & Lvwitems.SelectedItem & "','" & txtransdate & "'," & Lvwitems.SelectedItem.SubItems(4)
...
C = "Paybill"
If txtpaybill = "" Then
MsgBox "Plz enter paybill mpesa code."
Exit Sub
End If
...
sql = ""
sql = sql & "SET dateformat DMY INSERT INTO ag_Receipts(R_No, P_code, T_Date, Amount, Qua, S_Bal, user_id, Cash, SNo,Transby, Idno, mobile,remarks) VALUES ("
sql = sql & txtrno & ",'" & Lvwitems.SelectedItem & "','" & txtransdate & "'," & Lvwitems.SelectedItem.SubItems(4)
sql = sql & "," & Lvwitems.SelectedItem.SubItems(2) & "," & Remain & ",'" & User & "'," & Lvwitems.SelectedItem.SubItems(5) & ",'" & C & "','" & txttransby & "','" & txtIdNo & "','" & txtmobile & "','" & txtpaybill & "')"
...
'        txtFile.WriteLine "TOTAL" & vbTab & TXTTOTAL
        ...
        txtFile.WriteLine "Trans By" & vbTab & txttransby
        txtFile.WriteLine "Id No" & vbTab & txtIdNo
        txtFile.WriteLine "Paybill" & vbTab & txtpaybill
        txtFile.WriteLine "YOU WERE SERVED By " & UCase(username)
        txtFile.WriteLine "    Date :" & Format(Get_Server_Date, "dd/mm/yyyy HH:MM:SS AM/PM")
        txtFile.WriteLine "     THANK YOU AND WELCOME "
